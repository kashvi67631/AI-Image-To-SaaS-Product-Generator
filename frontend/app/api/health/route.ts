...
export async function GET(): Promise<Response> {
  ...
  try {
    const upstream = await fetchBackendWithEconnrefusedRetries(
      targetUrl,
      {
        method: "GET",
    ...
    if (isConnectionRefused) {
      console.error(
        ...
        detail.fullMessage,
        "\n  Fix checklist:",
        "1) From repo root: npm install && npm run dev (starts Express :8080 + Next together), OR second terminal: cd gemini-image-to-react-backend && npm run dev",
        "2) Backend log must show listening on 8080 — if another port appears, set BACKEND_URL to http://127.0.0.1:<that-port>",
        "3) frontend/.env.local: BACKEND_URL=http://127.0.0.1:8080 then restart Next",
      );
    } else {
